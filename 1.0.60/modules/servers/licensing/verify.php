<?php
###################                AQUI LAS LICENCIAS  DE LOS CLIENTES  By TEAM@ALPHA                  ##########################
$Licences = array('licenca1', 'licenca2', 'licenca3', 'licenca4', 'licenca5', 'licenca6');
...
if (!(isset($_POST['licensekey'])))
{
    ...
    {
        parse_str($data,$vars);
        if (in_array($vars['licensekey'], $Licences))
        {
            if (isset($vars['check_token']))
            {
                ...
                $result = '<status>Active</status>'
                    ...
                    . '<configoptions>Extra Load Balancer Servers=99</configoptions>\n'
...
{
    if (in_array($_POST['licensekey'], $Licences))
    {
        $md5Hash = (isset( $_POST["check_token"] ) ? md5( $md5_secret . $_POST["check_token"] ) : "");
        $result = '<status>Active</status>'
        . '<registeredname></registeredname>\n'
        . '<companyname></companyname>\n'
        . '<email></email>\n'
